...
public class ChaseTele extends OpMode {
    ...
    public void init() {
        ...
        servo2.setPosition(0); //Setting servo2 to start in initialization at start position, has to be between 0 and 1
    ...
    public void loop() {
        ...
        //Servos
        if (gamepad2.right_stick_x < 0 && servo1Pos != 1 && servo2Pos != 1) { //Reads if the right stick of the 2nd controller is being pushed left or right and if the servos postitions are not equal to 1
            servo1.setPosition(servo1Pos + .01); //Increases positions by .01 degree
            servo2.setPosition(servo2Pos + .01);

        } else if (gamepad2.right_stick_x > 0 && servo1Pos != 0 && servo2Pos != 0) { //Reads if the right stick of the 2nd controller is being pushed left or right and if the servos postitions are not equal to 0
            servo1.setPosition(servo1Pos - .01); //Decreases positions by .01 degree
            servo2.setPosition(servo1Pos - .01);
        }

        if (gamepad2.left_stick_y < 0 && servo3Pos != 1 && servo4Pos != 1) { //Reads if the gamepad left stick is pushed right/left, also if the servos position is equal to 1
            servo3.setPosition(+.01); //Increases positions by .01 degree
            servo4.setPosition(+.01);
        }
        else if(gamepad2.left_stick_y > 0 && servo3Pos != 0 && servo4Pos != 0) { //Reads if the gamepad left stick is pushed up/down, also if the servos position is equal to 0
            servo3.setPosition(-.01); //Decreases positions by .01 degree
            servo4.setPosition(-.01);
        }
                //Motors
